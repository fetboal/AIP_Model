...
#Get submission data for a given ticker
def get_submission_data_for_ticker(ticker, headers = headers, only_filings = False):
    cik = cik_matching_ticker(ticker, headers = headers)
    submission_url = f'https://data.sec.gov/submissions/CIK{cik}.json'
    company_json = requests.get(submission_url, headers = headers).json()
...
#Use this function to select what filings you want - Can adjust in future to include more options
def get_filtered_filings(ticker, form_types = ['10-K'], just_accession_number = False, headers = headers):
    company_fillings_df = get_submission_data_for_ticker(ticker, headers = headers, only_filings = True)
    
    df = company_fillings_df[company_fillings_df['form'].isin(form_types)]
    if just_accession_number:
...
def annual_facts(ticker, form_types=['10-K'], headers=headers):
    accession_nums = get_filtered_filings(
        ticker, form_types=form_types, just_accession_number=True
    )
    df, label_dict = facts_DF(ticker, headers)
    ten_k = df[df["accn"].isin(accession_nums)]
    accession_dates = pd.to_datetime(accession_nums.index)
    ten_k = ten_k[ten_k.index.isin(accession_dates)]
    pivot = ten_k.pivot_table(values="val", columns="fact", index="end")
    pivot.rename(columns=label_dict, inplace=True)
    return pivot.T

def quarterly_facts(ticker, form_types=['10-Q'], headers=headers):
    accession_nums = get_filtered_filings(
        ticker, form_types=form_types, just_accession_number=True
    )
    df, label_dict = facts_DF(ticker, headers)
    ...
    accession_dates = pd.to_datetime(accession_nums.index)
    ten_q = ten_q[ten_q.index.isin(accession_dates)].reset_index(drop=False)
...
def get_statement_file_names_in_filing_summary(ticker, accession_number, headers=None):
    ...
    try:
        ...
        for report in filing_summary_soup.find_all("Report"):
            ...
            short_tag, long_tag = report.find("ShortName"), report.find("LongName")
            if _is_statement_file(short_tag, long_tag, file_name):
                short_text = short_tag.get_text(strip=True) if short_tag and short_tag.get_text() else ""
                if short_text:
                    statement_file_names_dict[short_text.lower()] = file_name
